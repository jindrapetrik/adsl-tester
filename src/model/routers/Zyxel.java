...
public class Zyxel extends MeasuredRouter {
    ...
    protected void firstMeasure() throws IOException {
        ...
        String lines[];
        //WAN MTU,IP
        lines = sendCommand("ip ifconfig wanif1");
        ...
        //ES 24 h
        lines = sendCommand("wan adsl errorsecond show");
        if (lines.length > 0) {
        ...
        lines = sendCommand("wan node display");
        for (int i = 0; i < lines.length; i++) {
    ...
    public void doMeasurement() {
        try {
            ...
            measureStart();
            //Speed down
            String[] lines = sendCommand("wan adsl chandata");
            for (int i = 0; i < lines.length; i++) {
            ...
            //DSL Standard
            lines = sendCommand("wan adsl opmode");
            if (lines.length > 0) {
            ...
            //Graph
            lines = sendCommand("wan adsl linedata far");
            int tonestart = 0;
            for (int i = 0; i < lines.length; i++) {
            ...
            //Status
            lines = sendCommand("wan adsl status wan");
            if (lines.length > 0) {
                ADSLStatus = lines[0].substring(lines[0].indexOf(":") + 2);
            }
            //Margin down...
            lines = sendCommand("wan adsl linedata near");
            for (int i = 0; i < lines.length; i++) {
            ...
            //ATUC
            lines = sendCommand("wan adsl farituid");
            if (lines.length > 0) {
