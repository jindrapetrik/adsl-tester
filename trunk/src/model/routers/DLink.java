...
public class DLink extends MeasuredRouter {
    ...
    protected void firstMeasure() throws IOException {
        ...
        maxSpeedUp = "?";
        sysVersion = "?";
        lines = sendCommand("grep VERSION= /etc/versions");
        if (lines.length > 0) {
            sysVersion = lines[0].substring(8);
        }
        lines = sendCommand("echo \"begin;connection0:pppoe:settings/password;end\" | cm_cli");
        if (lines.length > 0) {
            password = lines[0];
        }
        lines = sendCommand("echo \"begin;connection0:pppoe:settings/username;end\" | cm_cli");
        if (lines.length > 0) {
            name = lines[0];
        }
        lines = sendCommand("echo \"begin;snmpcm:settings/system/sysname;end\" | cm_cli");
        if (lines.length > 0) {
            model = lines[0];
        }
        lines = sendCommand("ifconfig ppp0");
    ...
    public void doMeasurement() {
        ...
        try {
            ...
            if (!connected) {
                ...
                login();
                firstMeasure();
            /*sendLine("cat /etc/config.xml");
            readLine();
            String s="";
            try {
            FileOutputStream fos = new FileOutputStream("modemconfig.xml");
            ...
            lines = sendCommand("cat avsar_modem_stats");
            String s = "";
            for (int i = 0; i < lines.length; i++) {
            ...
            lines = sendCommand("cat avsar_bit_allocation_table");
            ...
            lines = sendCommand("cat avsar_channels");
            for (int i = 0; i < lines.length; i++) {
