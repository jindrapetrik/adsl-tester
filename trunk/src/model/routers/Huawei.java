...
public class Huawei extends MeasuredRouter {
    ...
    protected void firstMeasure() throws IOException{
        ...
        //WAN MTU,IP        
        lines=sendCommand("ip ifconfig wanif0");
        ...
        //ATTNDRds
        lines=sendCommand("wan dmt2 show rparams");
        for(int i=0;i<lines.length;i++){
        ...
        //ATTNDRus
        lines=sendCommand("wan dmt2 show cparams");
        for(int i=0;i<lines.length;i++){
    ...
    public void doMeasurement() {        
        try {
            ...
            measureStart();
            //Speed down
            String[] lines = sendCommand("show wan adsl chandata");
            if (lines.length >= 4) {
                try {
            ...
            //DSL Standard
            lines = sendCommand("show wan adsl opmode");
            if (lines.length > 0) {
                dslStandard = lines[0].substring(lines[0].indexOf(":") + 2);
            }
            //Graph
            lines = sendCommand("show wan adsl linedata far");
            int tonestart = 0;
            for (int i = 0; i < lines.length; i++) {
            ...
            //Status
            lines = sendCommand("show wan adsl status wan");
            if (lines.length > 0) {
                ADSLStatus = lines[0].substring(lines[0].indexOf(":") + 2);
            }
            //Margin down...
            lines = sendCommand("show wan adsl linedata near");
            for (int i = 0; i < lines.length; i++) {
            ...
            //Errors, uptime
            lines = sendCommand("wan adsl perfdata");
            for (int i = 0; i < lines.length; i++) {
                //up:
            ...
            lines = sendCommand("show wan node 0");
            for (int i = 0; i < lines.length; i++) {
            ...
            //Bridge, vpivci,...
            lines = sendCommand("show wan node 1");
            for (int i = 0; i < lines.length; i++) {
                if (lines[i].startsWith("Channel active")) {
            ...
            //ATUC
            lines = sendCommand("show wan adsl farituid");
            if (lines.length > 0) {
