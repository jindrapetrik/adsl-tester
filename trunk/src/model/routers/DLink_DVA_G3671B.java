    ...
    public void doMeasurement() throws IOException {
        ...
        for (int i = 0; i < lines.size(); i++) {
            ...
            if(lines.get(i).indexOf("HEC:")==0){
                ...
                tmp=tmp.substring(4);
                tmp=tmp.trim();
                errorsDownStreamFastHEC=tmp.substring(0,tmp.indexOf(" "));
                errorsUpStreamFastHEC=tmp.substring(tmp.lastIndexOf(" ")+1);
            }
            if(lines.get(i).indexOf("Total time = ")==0){
                ...
                if(tmp.indexOf("FEC:")==0){
                    tmp=tmp.substring(4);
                    tmp=tmp.trim();
                    errorsDownStreamFastFEC=tmp.substring(0,tmp.indexOf(" "));
                    errorsUpStreamFastFEC=tmp.substring(tmp.lastIndexOf(" ")+1);
                }
                tmp=lines.get(i+2);
                if(tmp.indexOf("CRC:")==0){
                    tmp=tmp.substring(4);
                    tmp=tmp.trim();
                    errorsDownStreamFastCRC=tmp.substring(0,tmp.indexOf(" "));
                    errorsUpStreamFastCRC=tmp.substring(tmp.lastIndexOf(" ")+1);
                }
